...
export default function Course() {
    ...
    <h1 className="courses-title align-top text-slate-900 font-bold text-3xl font-mono p-10">Available Courses</h1>
    <div className="grid grid-cols-2 gap-10 ">

      {data?.map((course) => (
        <div key={course.id} className="hover:scale-105 bg-white p-4 rounded shadow h-64">
          <h2 className="font-bold text-xl text-slate-900">{course.name}</h2>
          <p className="text-sm text-slate-500">{course.grade}</p>
        <label className="text-md flex content-center align-center text-slate-800">
            <input type="checkbox" checked={course.mathSN4} readOnly />
            Math SN4
        </label>
        <label className="text-md flex content-center align-center text-slate-800">
            <input type="checkbox" checked={course.mathSN5} readOnly />
            Math SN5
        </label>
        <label className="text-md flex content-center align-center text-slate-800">
            <input type="checkbox" checked={course.chimiePhysique} readOnly />
            Chimie/Physique
        </label>
        </div>
      ))}
