	...
	return (
        ...
        {isOpen && (
			<header className={`mt-4 bg-gray-300 rounded-md shadow-md shadow-white p-10 flex flex-col gap-6 fade-in  justify-around transition delay-500 ease-in-out duration-500 ${isOpen ? 'show' : ''}`}>
                ...
                <button className="text-slate-900 font-bold font-mono text-lg hover:scale-125">Home</button>
				<button  className="text-slate-900 font-bold font-mono text-lg hover:scale-125">Courses</button>
				<button  className="text-slate-900 font-bold font-mono text-lg hover:scale-125">About</button>
			...
			<main className="flex h-screen ">
				...
				<div className="bg-gray-200 md:p-10 lg:p-20 w-full flex flex-col  justify-center items-center h-screen   gap-10 ">
        			...
        			<input type="text" className="flex text-slate-900 relative border-2 border-gray-300 bg-white h-10 px-5 pr-16 rounded-lg text-sm focus:outline-none" placeholder="Search jobs..." />
